...
function resolveIndexOfflineAssetUrl(assetKey) {
...
function ensureIndexChartRuntimeLoaded() {
  ...
  let runtimeUrl = "";
  try {
    runtimeUrl = resolveIndexOfflineAssetUrl(INDEX_CHART_RUNTIME_KEY);
  } catch (error) {
...
function isIndexShellTransitionLoading() {
...
const TIMER_MODAL_SUGGESTION_FIRST_FOCUS_DELAY_MS = 48;
...
let modalProjectSelectionClickFallbackSuppressedUntil = {
  "project-name-input": 0,
  "next-project-input": 0,
};
...
const INDEX_RECORD_LOAD_MODE_RECENT_RANGE = "recent-range";
const INDEX_RECORD_LOAD_MODE_FULL_HISTORY = "full-history";
...
let indexInitialDataValidated = false;
let indexBootstrappedFromPageBootstrap = false;
...
function formatIndexScopeDateKey(date) {
...
function filterIndexRecordsByScope(items = [], scope = null) {
  const normalizedScope = cloneIndexRecordLoadScope(scope);
  if (!normalizedScope) {
    return Array.isArray(items) ? items.slice() : [];
  }
  const startDate = String(normalizedScope.startDate || "").trim();
  const endDate = String(normalizedScope.endDate || "").trim();
  if (!startDate && !endDate) {
    return Array.isArray(items) ? items.slice() : [];
  }
  return (Array.isArray(items) ? items : []).filter((record) => {
    const recordDateKey = formatIndexScopeDateKey(resolveRecordTime(record));
    if (!recordDateKey) {
      return false;
    }
    if (startDate && recordDateKey < startDate) {
      return false;
    }
    if (endDate && recordDateKey > endDate) {
      return false;
    }
    return true;
  });
...
function buildIndexWorkspacePerfDetail(snapshot = {}) {
  ...
  const snapshotPeriodIds = normalizeIndexRecordPeriodIdList(
    Array.isArray(snapshot.loadedPeriodIds)
      ...
      : getIndexRecordPeriodIds(snapshotRecords),
  );
  const snapshotScope = cloneIndexRecordLoadScope(snapshot.recordScope);
  return {
    projectCount: snapshotProjects.length,
    recordCount: snapshotRecords.length,
    periodIds: snapshotPeriodIds.slice(),
    loadedPeriodCount: snapshotPeriodIds.length,
    startDate: snapshotScope?.startDate || "",
    endDate: snapshotScope?.endDate || "",
    source: typeof snapshot.source === "string" ? snapshot.source : "",
    fromCache: true,
  };
}

function markIndexInitialDataReady(snapshot = {}) {
  const perfDetail = buildIndexWorkspacePerfDetail(snapshot);
  uiTools?.markPerfStage?.("first-data-ready", perfDetail);
  uiTools?.markPerfStage?.("index-snapshot-data-ready", perfDetail);
}
  ...
  return {
    ...
    loadedPeriodIds: normalizeIndexRecordPeriodIdList(
      Array.isArray(options.loadedPeriodIds)
        ...
        : getIndexRecordPeriodIds(rawRecords),
    ),
  };
}
...
function applyIndexWorkspaceSnapshot(snapshot = {}) {
  ...
  indexAllHistoricalRecordsLoaded =
    normalizeIndexRecordLoadMode(snapshot.recordLoadMode) ===
    INDEX_RECORD_LOAD_MODE_FULL_HISTORY;
  rememberIndexRecordLoadWindow(
    snapshot.recordLoadMode || INDEX_RECORD_LOAD_MODE_RECENT_RANGE,
    snapshot.recordScope || getIndexDefaultRecordScope(),
  );
  indexLoadedRecordPeriodIds = normalizeIndexRecordPeriodIdList(
    Array.isArray(snapshot.loadedPeriodIds) && snapshot.loadedPeriodIds.length
      ? snapshot.loadedPeriodIds
  ...
  emitIndexDebugPerf(
    ...
    buildIndexWorkspacePerfDetail({
      ...
      projects: nextProjects,
      records: nextRecords,
      loadedPeriodIds: indexLoadedRecordPeriodIds.slice(),
    }),
  );
}
...
function bootstrapIndexFromCachedSnapshot() {
  ...
  applyIndexWorkspaceSnapshot(snapshot);
  return {
    ...snapshot,
...
function cloneIndexRecordLoadScope(scope = null) {
  ...
  if (normalizedEndDate) {
    clonedScope.endDate = normalizedEndDate;
  }
  if (normalizedPeriodIds.length) {
    clonedScope.periodIds = normalizedPeriodIds;
  }
  return Object.keys(clonedScope).length ? clonedScope : null;
}

function getIndexCurrentRecordLoadOptions() {
  const mode = normalizeIndexRecordLoadMode(indexLoadedRecordWindowMode);
  if (mode === INDEX_RECORD_LOAD_MODE_FULL_HISTORY) {
    return {
      recordLoadMode: INDEX_RECORD_LOAD_MODE_FULL_HISTORY,
      recordScope: null,
    };
  }
  return {
    recordLoadMode: INDEX_RECORD_LOAD_MODE_RECENT_RANGE,
    recordScope:
      cloneIndexRecordLoadScope(indexLoadedRecordWindowScope) ||
      getIndexDefaultRecordScope(),
  };
}

function rememberIndexRecordLoadWindow(mode, scope = null) {
  indexLoadedRecordWindowMode = normalizeIndexRecordLoadMode(mode);
...
function beginIndexSaveTransaction() {
  const transactionId = ++indexSaveTransactionCounter;
  indexActiveSaveTransactionId = transactionId;
  return transactionId;
}

function isIndexSaveTransactionActive(transactionId = 0) {
...
function finishIndexSaveTransaction(transactionId = 0) {
  if (!transactionId || !isIndexSaveTransactionActive(transactionId)) {
    return false;
  }
  indexActiveSaveTransactionId = 0;
  flushDeferredIndexExternalRefreshIfNeeded();
  return true;
}
...
async function refreshIndexFromExternalStorageChange() {
  ...
  try {
    const shouldManageRefreshLoading = !indexInitialDataLoaded;
    ...
    const refreshResult = await indexRefreshController.run(
        ...
        }),
      {
        ...
        loadingOptions: {
          mode: shouldManageRefreshLoading ? "fullscreen" : "inline",
...
function restoreIndexSpendModalInteractivityIfIdle(reason = "manual") {
  if (isModalOpen || indexModalConfirmPending) {
    return false;
  }
...
function syncIndexProjectMirrorIfNeeded(projectList = projects) {
  ...
  try {
    const previousSnapshot = getIndexRawLocalMirrorItem("projects");
    if (previousSnapshot === snapshot) {
      return false;
    }
    setIndexRawLocalMirrorItem("projects", snapshot);
    return true;
  } catch (error) {
...
function refreshTimerSessionModalBaselineSnapshot() {
  if (!isModalOpen) {
    return false;
  }
...
function commitPrimaryModalProjectInput(options = {}) {
  ...
  if (!matchedProject && allowCreate === true) {
    ...
    matchedProject =
      projects.find((project) => project.name === creatableName) || null;
  }

  const storedValue = matchedProject ? matchedProject.name : rawValue;
  selectedProject = storedValue;
  lastEnteredProjectName = storedValue;
  if (matchedProject) {
    setProjectInputValue("project-name-input", matchedProject.name);
  } else {
    projectNameInput.value = rawValue;
  }
  updateProjectsList();
  updateExistingProjectsList();
  renderProjectSuggestionsForInput(
...
function hasPendingTimerModalSoftInputRequest(target) {
  if (!(target instanceof HTMLElement)) {
    return false;
  }
  return Number(target.__controlerAndroidSoftInputRequestPendingUntil || 0) > Date.now();
}

function shouldScheduleTimerModalProjectInputFocus(inputId) {
  ...
  if (document.activeElement === input && input.matches?.(":focus")) {
    return !isTimerModalAndroidKeyboardOpen();
  }

  const activeTextEntry = getActiveTimerModalTextEntry();
  if (activeTextEntry instanceof HTMLElement && activeTextEntry !== input) {
    return true;
  }
...
function collapseTimerSessionFieldRevealSpacing(modalBody) {
  if (!(modalBody instanceof HTMLElement)) {
    return false;
  }
  modalBody.style.removeProperty("--controler-form-modal-body-extra-bottom-space");
  const revealState = modalBody.__controlerTimerSessionFieldRevealState;
...
function isTimerModalProjectSuggestionVisible(inputId) {
  ...
  return popover instanceof HTMLElement && popover.classList.contains("visible");
}

function hasVisibleTimerModalProjectSuggestions() {
  return TIMER_MODAL_PROJECT_INPUT_IDS.some((inputId) =>
    isTimerModalProjectSuggestionVisible(inputId),
  );
}

function clearTimerModalSuggestionDismissPreserveState() {
  timerModalSuggestionPreserveInputId = "";
  timerModalSuggestionPreserveUntil = 0;
  timerModalSkipNextOutsideSuggestionHideUntil = 0;
}

function markTimerModalSuggestionDismissPreserve(inputId, windowMs = 420) {
  if (!isTimerModalProjectInputId(inputId)) {
    return false;
  }
  const now = Date.now();
  const safeWindowMs = Math.max(180, Number(windowMs) || 420);
  timerModalSuggestionPreserveInputId = inputId;
  timerModalSuggestionPreserveUntil = now + safeWindowMs;
  timerModalSkipNextOutsideSuggestionHideUntil = now + safeWindowMs;
  return true;
}

function hasRecentTimerModalSuggestionDismissPreserve(inputId, now = Date.now()) {
  if (
  ...
  if (timerModalSuggestionPreserveUntil <= now) {
...
function shouldSkipNextTimerModalSuggestionOutsideHide(now = Date.now()) {
  if (timerModalSkipNextOutsideSuggestionHideUntil <= now) {
    timerModalSkipNextOutsideSuggestionHideUntil = 0;
    return false;
  }
  timerModalSkipNextOutsideSuggestionHideUntil = 0;
...
function blurActiveTimerModalTextEntry(options = {}) {
  ...
  if (
    ...
    typeof uiTools?.releaseAndroidInteractiveTextControlFocus === "function"
  ) {
...
function isTimerModalBlankDismissTarget(target) {
  if (!(target instanceof HTMLElement)) {
    return false;
  }
...
function suppressModalProjectSelectionClickFallback(inputId, windowMs = 0) {
  ...
  modalProjectSelectionClickFallbackSuppressedUntil[inputId] =
    Date.now() + Math.max(0, Number(windowMs) || 0);
  return true;
}

function clearModalProjectSelectionClickFallbackSuppression(inputId) {
  if (!isTimerModalProjectInputId(inputId)) {
    return false;
  }
  modalProjectSelectionClickFallbackSuppressedUntil[inputId] = 0;
  return true;
}

function isModalProjectSelectionClickFallbackSuppressed(inputId) {
  if (!isTimerModalProjectInputId(inputId)) {
    return false;
  }
  return (
    Date.now() <
    (Number(modalProjectSelectionClickFallbackSuppressedUntil[inputId]) || 0)
  );
}
...
function shouldPreserveTimerModalSuggestionsOnKeyboardDismiss(input, inputId) {
  ...
  if (!isTimerModalProjectSuggestionVisible(inputId)) {
    return false;
  }
  ...
  const now = Date.now();
  const preservedByKeyboardDismiss =
    dismissedAt > 0 && now - dismissedAt <= 1200;
  return (
    preservedByKeyboardDismiss ||
    hasRecentTimerModalSuggestionDismissPreserve(inputId, now)
  );
}
...
function bindTimerModalProjectSelectionTarget(
  ...
  const moveThresholdPx = Math.max(8, Number(options.moveThresholdPx) || 10);
  const maxTapDurationMs = Math.max(
    180,
    Number(options.maxTapDurationMs) || 320,
  );
  let pressSelectionPending = null;
  let lastSelectionCommitAt = 0;
  ...
  const prepareInteraction = (event) => {
    ...
    if (event?.type !== "pointerdown") {
      pressSelectionPending = null;
      return;
    }
    pressSelectionPending = {
  ...
  const commitSelection = (event) => {
    const now = Date.now();
    if (now - lastSelectionCommitAt < 360) {
      return false;
    }
    lastSelectionCommitAt = now;
    event?.preventDefault?.();
    event?.stopPropagation?.();
    ...
    return applyTimerModalProjectSelection(inputId, selection, {
      manual: true,
    });
  };

  const shouldCommitPendingInteraction = (event) => {
    if (!pressSelectionPending) {
      return false;
    }
    if (
      Number.isFinite(pressSelectionPending.pointerId) &&
      Number.isFinite(event?.pointerId) &&
      pressSelectionPending.pointerId !== event.pointerId
    ) {
      return false;
    }
    const deltaX = Math.abs(
      (Number.isFinite(event?.clientX) ? event.clientX : 0) -
        pressSelectionPending.startX,
    );
    const deltaY = Math.abs(
      (Number.isFinite(event?.clientY) ? event.clientY : 0) -
        pressSelectionPending.startY,
    );
  ...
  const handlePointerMove = (event) => {
    ...
    if (!shouldCommitPendingInteraction(event)) {
      pressSelectionPending = null;
    }
  };

  const commitPressSelection = (event) => {
    lastPointerInteractionAt = Date.now();
    const shouldCommit = shouldCommitPendingInteraction(event);
    pressSelectionPending = null;
    if (!shouldCommit) {
      return;
    }
  ...
  target.addEventListener("pointerup", commitPressSelection);
  target.addEventListener("pointercancel", resetPressSelection);
  target.addEventListener("click", (event) => {
    if (
      typeof window.PointerEvent === "function" &&
      Date.now() - lastPointerInteractionAt < 420
    ) {
      event.preventDefault?.();
      event.stopPropagation?.();
      if (typeof event.stopImmediatePropagation === "function") {
    ...
    if (isModalProjectSelectionClickFallbackSuppressed(inputId)) {
      event.preventDefault?.();
      event.stopPropagation?.();
      if (typeof event.stopImmediatePropagation === "function") {
        event.stopImmediatePropagation();
...
function getTimerModalSelectionFollowupSuggestionsKeyword(
  targetInputId,
  selectedProject,
  selectedPath,
) {
  if (
    ...
    !selectedProject ||
    normalizeProjectLevel(selectedProject.level) !== 1
  ) {
    return "";
  }

  const hierarchyIndex = getIndexProjectHierarchyIndex(projects);
  const childProjects = getIndexHierarchyChildren(
    hierarchyIndex,
...
function applyTimerModalProjectSelection(
  ...
  if (followupSuggestionsKeyword) {
    // The tap that picked the root project can otherwise "fall through" into
    // the freshly rendered child list and instantly select another item.
    suppressModalProjectSelectionClickFallback(targetInputId, 480);
    clearModalProjectSuggestionHideTimer(targetInputId);
    renderProjectSuggestionsForInput(
      targetInputId,
      followupSuggestionsKeyword,
      true,
    );
  }
  return true;
...
function scheduleTimerModalProjectInputFocus(targetInputId, options = {}) {
  ...
  const runFocus = () => {
    ...
    const targetAlreadyFocused =
      document.activeElement === targetInput && targetInput.matches?.(":focus");
    if (targetAlreadyFocused && isTimerModalAndroidKeyboardOpen()) {
      return;
    }
    if (targetAlreadyFocused && hasPendingTimerModalSoftInputRequest(targetInput)) {
      return;
    }
...
function scheduleTimerSessionFieldReveal(target, options = {}) {
  ...
  const anchorContainer =
    target.closest(".input-with-suggestions") ||
    target.closest(".form-group") ||
    revealAnchor;
  if (
    !(modalBody instanceof HTMLElement) ||
    !(revealAnchor instanceof HTMLElement) ||
    !(anchorContainer instanceof HTMLElement)
  ) {
    return false;
  }
...
function requestSpendModalOpen(requestedClickTime = new Date(), options = {}) {
  ...
  if (
    !isModalOpen &&
    !indexModalConfirmPending &&
    (spendModalClickLocked || pendingSpendModalState) &&
    now - lastSpendButtonAcceptedAt >= 420
  ) {
    restoreIndexSpendModalInteractivityIfIdle("stale-open-request");
  }
  if (
...
function initIndexModalBindings() {
  ...
  const bindProjectInputEvents = (input, inputId) => {
    ...
    input.addEventListener("pointerdown", (event) => {
      ...
      clearPendingTimerModalFocusRequest();
      const shouldScheduleFocus =
        shouldScheduleTimerModalProjectInputFocus(inputId);
      const inputAlreadyFocused =
        document.activeElement === input && input.matches?.(":focus");
      setModalProjectInputTarget(inputId, {
        manual: true,
        showSuggestions: shouldScheduleFocus || inputAlreadyFocused,
      });
  ...
  const timerModalBody = document.querySelector(
    "#modal-overlay .controler-form-modal-body",
  );
  if (timerModalBody instanceof HTMLElement) {
    const blankTapThresholdPx = 12;
    const blankTapMaxDurationMs = 320;
    let blankTapGesture = null;

    const clearBlankTapGesture = () => {
      blankTapGesture = null;
    };

    const isEligibleBlankTap = (event) => {
      const target = event?.target;
      if (!isModalOpen || !(target instanceof HTMLElement)) {
        return false;
      }
      if (!isTimerModalBlankDismissTarget(target)) {
        return false;
      }
      return (
        getActiveTimerModalTextEntry() instanceof HTMLElement ||
        hasVisibleTimerModalProjectSuggestions()
      );
    };

    timerModalBody.addEventListener("pointerdown", (event) => {
      if (!isEligibleBlankTap(event)) {
        clearBlankTapGesture();
        return;
      }
      blankTapGesture = {
        pointerId: Number.isFinite(event.pointerId) ? event.pointerId : null,
    ...
    timerModalBody.addEventListener("pointermove", (event) => {
      if (!blankTapGesture) {
        return;
      }
      if (
        Number.isFinite(blankTapGesture.pointerId) &&
        Number.isFinite(event.pointerId) &&
        blankTapGesture.pointerId !== event.pointerId
      ) {
        clearBlankTapGesture();
        return;
      }
      const deltaX = Math.abs(
        (Number.isFinite(event.clientX) ? event.clientX : 0) -
          blankTapGesture.startX,
      );
      const deltaY = Math.abs(
        (Number.isFinite(event.clientY) ? event.clientY : 0) -
          blankTapGesture.startY,
      );
      if (deltaX > blankTapThresholdPx || deltaY > blankTapThresholdPx) {
        clearBlankTapGesture();
      }
    });

    timerModalBody.addEventListener("pointercancel", clearBlankTapGesture);
    timerModalBody.addEventListener("scroll", clearBlankTapGesture, {
      passive: true,
    });
    timerModalBody.addEventListener("pointerup", (event) => {
      if (!blankTapGesture) {
        return;
      }
      ...
      const elapsedMs =
        (Number(event.timeStamp) || Date.now()) - blankTapGesture.startedAt;
      clearBlankTapGesture();
      if (
        !pointerMatches ||
        deltaX > blankTapThresholdPx ||
        deltaY > blankTapThresholdPx ||
        elapsedMs > blankTapMaxDurationMs
      ) {
        return;
      }
      const activeTextEntry = getActiveTimerModalTextEntry();
      ...
      if (
        ...
        markTimerModalSuggestionDismissPreserve(activeProjectInputId);
        blurActiveTimerModalTextEntry({
          fallbackTarget: activeTextEntry,
        });
        return;
      }
      if (hasVisibleTimerModalProjectSuggestions()) {
        hideAllProjectSuggestions();
      }
      blurActiveTimerModalTextEntry({
        fallbackTarget: activeTextEntry,
  ...
  const revealFocusedTimerModalField = (delayMs = 0) => {
    if (!isModalOpen) {
      return;
    }
    const activeTarget = getActiveTimerModalTextEntry();
    if (activeTarget instanceof HTMLElement) {
      scheduleTimerSessionFieldReveal(activeTarget, {
        delayMs,
      });
      return;
    }
    ...
    if (isTimerModalAndroidKeyboardOpen()) {
      syncTimerSessionFieldRevealSpacing(timerModalBody);
      return;
    }
    collapseTimerSessionFieldRevealSpacing(timerModalBody);
  };
  ...
  document.addEventListener("click", (event) => {
    ...
    if (shouldSkipNextTimerModalSuggestionOutsideHide()) {
      return;
    }
...
async function handleIndexModalConfirmClick() {
  ...
  try {
    ...
    const predictedNextClickCount = Number.isFinite(pendingBaseState?.ptn)
    ...
    if (willPersistRecord) {
      ...
      uiTools?.markPerfStage?.("record-save-modal-hidden", {
        allowRepeat: true,
        createdProjectPendingSave,
      });
    }
...
function restoreIndexWorkspacePersistenceSnapshot(snapshot) {
  ...
  indexLoadedRecordPeriodIds =
    ...
    snapshot.loadedRecordPeriodIds.length
      ...
      : getIndexRecordPeriodIds(records);
  rememberIndexRecordLoadWindow(
    snapshot.loadedRecordWindowMode,
    snapshot.loadedRecordWindowScope,
  );
  indexDirtyRecordPeriodIds = new Set(
  ...
  updateExistingProjectsList();
  updateParentProjectSelect(1);
  restoreIndexSpendModalInteractivityIfIdle("workspace-restore");
  refreshIndexWorkspace({ immediate: true });
  return true;
}
...
async function recoverIndexWorkspaceAfterPersistenceFailure(
  ...
  try {
    ...
    if (!indexShellPageActive) {
      indexExternalRefreshPendingResume = true;
      clearIndexPersistenceError();
      restoreIndexSpendModalInteractivityIfIdle("recovery-hidden-page");
      return true;
    }
    await hydrateIndexWorkspace({
    ...
    resetIndexRecordPersistenceTracking();
    clearIndexPersistenceError();
    restoreIndexSpendModalInteractivityIfIdle("recovery-success");
    return true;
  } catch (error) {
...
async function flushIndexPendingPersistenceOrThrow(
  options = {},
  message = "记录页数据保存失败",
) {
  const persisted = await flushIndexPendingPersistence(options);
  if (!persisted) {
    throw new Error(String(message || "记录页数据保存失败"));
  }
  return true;
}
...
function getIndexRawLocalMirrorStorage() {
  const storageApi = window.ControlerStorage;
  if (
    storageApi &&
    typeof storageApi.getRawLocalItem === "function" &&
    typeof storageApi.setRawLocalItem === "function" &&
    typeof storageApi.removeRawLocalItem === "function"
  ) {
    return storageApi;
  }
  return null;
...
function getIndexRawLocalMirrorItem(key) {
  const storageApi = getIndexRawLocalMirrorStorage();
  if (storageApi) {
    return storageApi.getRawLocalItem(key) || "";
  }
  return localStorage.getItem(key) || "";
}

function setIndexRawLocalMirrorItem(key, value) {
  const storageApi = getIndexRawLocalMirrorStorage();
  if (storageApi) {
    storageApi.setRawLocalItem(key, value);
    return;
  }
  localStorage.setItem(key, value);
}

function removeIndexRawLocalMirrorItem(key) {
  const storageApi = getIndexRawLocalMirrorStorage();
  if (storageApi) {
    storageApi.removeRawLocalItem(key);
    return;
  }
...
function readIndexProjectMirrorState() {
  try {
    const raw = getIndexRawLocalMirrorItem("projects");
    if (!raw) {
      return {
        items: [],
        hasMirror: false,
...
// 从localStorage加载记录
async function loadRecordsFromStorage(options = {}) {
  try {
    ...
    if (managedStorage) {
      ...
      fallbackRecords = Array.isArray(mirrorItems) ? mirrorItems.slice() : [];
    }
    ...
    if (typeof window.ControlerStorage?.loadSectionRange === "function") {
      ...
      const resolvedLoad =
        typeof indexRecordPersistenceApi?.resolveRecordLoadResult === "function"
          ? indexRecordPersistenceApi.resolveRecordLoadResult({
              mode: loadMode,
              existingItems: existingRecordsSnapshot,
              fallbackItems: fallbackRecords,
              rangeItems,
              rangePeriodIds,
              getPeriodId: getIndexRecordPeriodId,
              mergeByPeriods: mergeIndexRecordsByPeriods,
            })
          : {
              items:
                loadMode === INDEX_RECORD_LOAD_MODE_RECENT_RANGE
      ...
      fallbackRecords = Array.isArray(resolvedLoad?.items)
        ? resolvedLoad.items.slice()
        : [];
      loadedPeriodIds = Array.isArray(resolvedLoad?.loadedPeriodIds)
        ? resolvedLoad.loadedPeriodIds.slice()
        : [];
    } else if (loadMode === INDEX_RECORD_LOAD_MODE_FULL_HISTORY) {
      loadedPeriodIds = getIndexRecordPeriodIds(fallbackRecords);
    }
    if (
